# 🌐 Browser Automation — Referencia Técnica

> Carga este archivo cuando el agente necesite implementar automatización web en un agente autónomo.
...
class VisualAgent:
    ...
    async def find_and_click(self, description: str) -> ToolResult:
        ...
        response = self.llm.chat([{
            "role": "user",
            "content": [
                {"type": "text", "text": f'Find the element matching: "{description}". Return coords as JSON: {{"x": number, "y": number}}'},
                {"type": "image", "data": screenshot}
            ]
        }])
...
| Approach | Cuándo | Limitación |
|----------|--------|-----------|
| Selector CSS (`click(selector)`) | Elemento con ID/clase estable | Frágil si cambia el HTML |
| Visual (`find_and_click(desc)`) | UI dinámica o sin selectores | Más lento, requiere modelo vision |
| `get_page_content()` | Extraer texto sin renderizar JS | No captura SPAs correctamente |
